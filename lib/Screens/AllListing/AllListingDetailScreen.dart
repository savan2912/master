...
import 'package:gotilo_new/Api/Response/AllListings/ResponseReserveBook.dart' hide Amenities;
...
import 'package:gotilo_new/Screens/AllListing/AllList/HotelDetailScreenForBook.dart';
...
import 'AllList/AdditionalServiceAddScreen.dart';
...
class AllListingDetailScreen extends StatefulWidget {
  final int? listId;
  const AllListingDetailScreen({super.key, this.listId = 0});
...
class _AllListingDetailScreenState extends State<AllListingDetailScreen>
  ...
  // Reserve Room Controllers
  DateTime? _checkInDate = DateTime.now();
  DateTime? _checkOutDate = DateTime.now().add(const Duration(days: 12));
  int _adultsCount = 1;
  int _childsCount = 0;
    ...
    return TweenAnimationBuilder<double>(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            _buildInfoRow(),
            if (myDealsList.isNotEmpty) const SizedBox(height: 35),
            if (myDealsList.isNotEmpty) _sectionHeader("Exclusive Deals"),
            const SizedBox(height: 15),
            if (myDealsList.isNotEmpty) _buildMyDealsList(myDealsList),
            if (amenities.isNotEmpty) const SizedBox(height: 40),
            if (amenities.isNotEmpty) _sectionHeader("Amenities"),
            const SizedBox(height: 15),
            if (amenities.isNotEmpty) _buildAmenitiesGrid(),
            const SizedBox(height: 40),
            if (vendor != null) _sectionHeader("Business Owner"),
            const SizedBox(height: 15),
            _buildVendorCard(),
            const SizedBox(height: 40),
            ...
            if (listDetail?.address != "" && listDetail?.address != null) const SizedBox(height: 40),
            if (listDetail?.address != "" && listDetail?.address != null) _sectionHeader("Location"),
            if (listDetail?.address != "" && listDetail?.address != null) const SizedBox(height: 12),
            if (listDetail?.address != "" && listDetail?.address != null)
              _buildDetailCard(Icons.location_on_rounded, "${listDetail?.address}"),
            const SizedBox(height: 12),
            if(isEvent==1)
            _sectionHeader("Event Booking"),
            if(isEvent==1)
            const SizedBox(height: 12),
            if(isEvent==1)
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed:() {
                  if(AppPrefs.userId != ""){
                    Get.to(()=> EventBookingScreen(listingId: widget.listId.toString(),));
                  }else{
                    SharedWidgets.showTopSnackBar(context, message:"Please Login First", title: "fail");
                  }
  ...
  Widget _buildReserveRoomSection() {
                  ...
                  children: [
                    ...
                    InkWell(
                      ...
                      child: Container(
                        ...
                        child: Text(
                          _checkInDate != null ? formatter.format(_checkInDate!) : "Select Date",
                          style: GoogleFonts.plusJakartaSans(
                            fontSize: 14,
                  ...
                  children: [
                    ...
                    InkWell(
                      ...
                      child: Container(
                        ...
                        child: Text(
                          _checkOutDate != null ? formatter.format(_checkOutDate!) : "Select Date",
                          style: GoogleFonts.plusJakartaSans(
                            fontSize: 14,
                            fontWeight: FontWeight.w500,
                            color: const Color(0xFF334155),
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: TextField(
                        ...
                        controller: TextEditingController(text: "$_adultsCount"),
                        onChanged: (val) {
                          _adultsCount = int.tryParse(val) ?? 1;
                        },
                      ),
                    ),
                  ],
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: TextField(
                        keyboardType: TextInputType.number,
                        decoration: const InputDecoration(border: InputBorder.none),
                        controller: TextEditingController(text: "$_childsCount"),
                        onChanged: (val) {
                          _childsCount = int.tryParse(val) ?? 0;
                        },
                      ),
                    ),
                  ],
      ...
      itemBuilder: (context, index) {
        ...
        return Container(
          ...
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              SvgPicture.asset("assets/amenities.svg", height: 40, width: 40),
              const SizedBox(height: 8),
              Padding(
  ...
  // Updated Vendor Card with "Book Service Now" Button
        ...
        children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const Text("Founder & Owner", style: TextStyle(color: Colors.grey, fontSize: 13)),
                  ],
                ),
              ),
              GestureDetector(
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed:() {
                if(AppPrefs.userId != ""){
                  Get.to(()=> AdditionalServiceAddScreen(listingId: widget.listId.toString(),));
                }else{
                  SharedWidgets.showTopSnackBar(context, message:"Please Login First", title: "fail");
                }
  ...
  Future<void> _callSubCategoryListDetails() async {
    try {
      ...
      ResponseSubcategoryListDetails? response = await ApiCalls.callSubCategoryListDetails(
      ...
      if (response != null &&
        ...
        events?.addAll(response.data!.events!);
        ...
        isService=response.data!.isServiceBooking;
        isHotel=response.data!.isHotelBooking;
        isEvent=response.data!.isEvent;
  ...
  Future<void> _callReserveBook() async {
    MyApplication.checkInternet().then((internet) async {
      if(internet){
        try{
          ...
          ResponseReserveBook? response = await ApiCalls.callReserveBook(RequestReserveBook(
            ...
            checkin: _checkInDate != null ? apiDateFormat.format(_checkInDate!) : "",
            checkout: _checkOutDate != null ? apiDateFormat.format(_checkOutDate!) : "",
          ));
